...
Tree newNode(Key word) {
   Tree new = malloc(sizeof(t_Node));
   assert(new != NULL);
   data(new) = word;
...
bool TreeSearch(Tree t, Key word) {
   if (t == NULL)
      return false;
...
Tree TreeInsert(Tree t, Key word, char *Url_File) {
   if(t != NULL && strcmp(word, data(t)) == 0)
      insertLL(t->Url_List, Url_File); 
   if (t == NULL){
      t = newNode(word);
      t->Url_List = insertLL(t->Url_List, Url_File);
   } 
   else if (strcmp(word , data(t)) < 0)
      left(t) = TreeInsert(left(t), word, Url_File);
   else if (strcmp(word , data(t)) > 0)
      right(t) = TreeInsert(right(t), word, Url_File);
   return t;
}
